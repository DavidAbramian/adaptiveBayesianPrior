...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function runMCMC_4DIR(dS,estimationMethod,VBMethod,samplingMethod)
%% Run
...
SPMMatPath = fullfile(outputPath, actualSubjStr, VBMethod);
load(fullfile(SPMMatPath,'SPM.mat'), 'SPM');

% Create output folder
resultsPath = fullfile(outputPath, actualSubjStr, estimationMethod);
mkdir(resultsPath);
...
%     niter = 10; % nbr of Gibbs iterations
%     warmup = 1;
...
%     rngseed = 102;
    ...
    % Gibbs loop
    nIterTotal = warmup + niter;
    
    tic
    
    %%% Print progress %%%%
    relevantVar = nIterTotal;
    nDigits = length(num2str(relevantVar));
    s = ['%',num2str(nDigits),'i/%',num2str(nDigits),'i'];
    fprintf('Slice %i, running MCMC... ', iLb)
    for i = 2:(warmup+niter)
        if i == 2
            fprintf(s, i, relevantVar)
        else
            fprintf([repmat('\b',1,2*nDigits+1), s], i, relevantVar);
        end
        %%% Print progress %%%
        
        if mod(i,plot_period) == 0
        ...
        if isGLMNoiseAR
            ...
            elseif strcmp(samplingMethod,'PCG') 
                ...
                try
                    ...
                    cholBData = chol(BData + speye(length(BData))*1e-10);
                end
...
%                     cholJData = chol(JData + speye(length(JData))*1e-10);
                    cholJData = chol((JData+JData')/2);
                end
...
save(fullfile(outputPath, ['sub-',subjStr], estimationMethod, 'MCMC.mat'),'MCMC','-v7.3');
save(fullfile(outputPath, ['sub-',subjStr], estimationMethod, 'everything.mat'));
disp('done.');
